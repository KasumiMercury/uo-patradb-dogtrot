...
//		GroupBy(channel.FieldDisplayName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Channel.Query().
//		Select(channel.FieldDisplayName).
//		Scan(ctx, &v)
...
func (cq *ChannelQuery) loadVideos(ctx context.Context, query *VideoQuery, nodes []*Channel, init func(*Channel), assign func(*Channel, *Video)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(channel.VideosTable)
		s.Join(joinT).On(s.C(video.FieldID), joinT.C(channel.VideosPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(channel.VideosPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(channel.VideosPrimaryKey[1]))
