...
// DescriptionChangeCreate is the builder for creating a DescriptionChange entity.
...
func (dcc *DescriptionChangeCreate) SetID(s string) *DescriptionChangeCreate {
	dcc.mutation.SetID(s)
	return dcc
}

// SetNillableID sets the "id" field if the given value is not nil.
func (dcc *DescriptionChangeCreate) SetNillableID(s *string) *DescriptionChangeCreate {
	if s != nil {
		dcc.SetID(*s)
	}
	return dcc
...
func (dcc *DescriptionChangeCreate) SetDescriptionID(id string) *DescriptionChangeCreate {
...
// Save creates the DescriptionChange in the database.
func (dcc *DescriptionChangeCreate) Save(ctx context.Context) (*DescriptionChange, error) {
	dcc.defaults()
	return withHooks(ctx, dcc.sqlSave, dcc.mutation, dcc.hooks)
}

// SaveX calls Save and panics if Save returns an error.
func (dcc *DescriptionChangeCreate) SaveX(ctx context.Context) *DescriptionChange {
	v, err := dcc.Save(ctx)
	if err != nil {
		panic(err)
	}
...
func (dcc *DescriptionChangeCreate) defaults() {
	if _, ok := dcc.mutation.ChangedAt(); !ok {
		v := descriptionchange.DefaultChangedAt()
		dcc.mutation.SetChangedAt(v)
	}
	if _, ok := dcc.mutation.ID(); !ok {
		v := descriptionchange.DefaultID()
...
func (dcc *DescriptionChangeCreate) check() error {
	if _, ok := dcc.mutation.Raw(); !ok {
		return &ValidationError{Name: "raw", err: errors.New(`ent: missing required field "DescriptionChange.raw"`)}
	}
	if v, ok := dcc.mutation.Raw(); ok {
...
func (dcc *DescriptionChangeCreate) sqlSave(ctx context.Context) (*DescriptionChange, error) {
	...
	if _spec.ID.Value != nil {
		if id, ok := _spec.ID.Value.(string); ok {
			...
			return nil, fmt.Errorf("unexpected DescriptionChange.ID type: %T", _spec.ID.Value)
		}
	}
...
func (dcc *DescriptionChangeCreate) createSpec() (*DescriptionChange, *sqlgraph.CreateSpec) {
	var (
		_node = &DescriptionChange{config: dcc.config}
		_spec = sqlgraph.NewCreateSpec(descriptionchange.Table, sqlgraph.NewFieldSpec(descriptionchange.FieldID, field.TypeString))
	)
...
// DescriptionChangeCreateBulk is the builder for creating many DescriptionChange entities in bulk.
...
// Save creates the DescriptionChange entities in the database.
func (dccb *DescriptionChangeCreateBulk) Save(ctx context.Context) ([]*DescriptionChange, error) {
	if dccb.err != nil {
		return nil, dccb.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(dccb.builders))
	nodes := make([]*DescriptionChange, len(dccb.builders))
...
func (dccb *DescriptionChangeCreateBulk) SaveX(ctx context.Context) []*DescriptionChange {
	v, err := dccb.Save(ctx)
	if err != nil {
		panic(err)
	}
