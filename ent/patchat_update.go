...
// PatChatUpdate is the builder for updating PatChat entities.
...
func (pcu *PatChatUpdate) Where(ps ...predicate.PatChat) *PatChatUpdate {
...
func (pcu *PatChatUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(patchat.Table, patchat.Columns, sqlgraph.NewFieldSpec(patchat.FieldID, field.TypeString))
	...
	if value, ok := pcu.mutation.AddedMagnitude(); ok {
		_spec.AddField(patchat.FieldMagnitude, field.TypeFloat64, value)
	}
	...
	if value, ok := pcu.mutation.AddedScore(); ok {
		_spec.AddField(patchat.FieldScore, field.TypeFloat64, value)
	}
...
// PatChatUpdateOne is the builder for updating a single PatChat entity.
...
// Where appends a list predicates to the PatChatUpdate builder.
func (pcuo *PatChatUpdateOne) Where(ps ...predicate.PatChat) *PatChatUpdateOne {
...
// Save executes the query and returns the updated PatChat entity.
func (pcuo *PatChatUpdateOne) Save(ctx context.Context) (*PatChat, error) {
	return withHooks(ctx, pcuo.sqlSave, pcuo.mutation, pcuo.hooks)
}

// SaveX is like Save, but panics if an error occurs.
func (pcuo *PatChatUpdateOne) SaveX(ctx context.Context) *PatChat {
...
func (pcuo *PatChatUpdateOne) sqlSave(ctx context.Context) (_node *PatChat, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "PatChat.id" for update`)}
	}
	...
	_node = &PatChat{config: pcuo.config}
