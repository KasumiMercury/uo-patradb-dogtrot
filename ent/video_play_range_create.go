...
// VideoPlayRangeCreate is the builder for creating a Video_play_range entity.
...
func (vprc *VideoPlayRangeCreate) SetID(pu pulid.ID) *VideoPlayRangeCreate {
	vprc.mutation.SetID(pu)
	return vprc
}

// SetNillableID sets the "id" field if the given value is not nil.
func (vprc *VideoPlayRangeCreate) SetNillableID(pu *pulid.ID) *VideoPlayRangeCreate {
	if pu != nil {
		vprc.SetID(*pu)
	}
	return vprc
...
// Save creates the Video_play_range in the database.
func (vprc *VideoPlayRangeCreate) Save(ctx context.Context) (*Video_play_range, error) {
	vprc.defaults()
	return withHooks(ctx, vprc.sqlSave, vprc.mutation, vprc.hooks)
}

// SaveX calls Save and panics if Save returns an error.
func (vprc *VideoPlayRangeCreate) SaveX(ctx context.Context) *Video_play_range {
	v, err := vprc.Save(ctx)
	if err != nil {
		panic(err)
	}
...
func (vprc *VideoPlayRangeCreate) defaults() {
	if _, ok := vprc.mutation.StartSeconds(); !ok {
		v := video_play_range.DefaultStartSeconds
		vprc.mutation.SetStartSeconds(v)
	}
	if _, ok := vprc.mutation.ID(); !ok {
		v := video_play_range.DefaultID()
...
func (vprc *VideoPlayRangeCreate) sqlSave(ctx context.Context) (*Video_play_range, error) {
	...
	if _spec.ID.Value != nil {
		if id, ok := _spec.ID.Value.(pulid.ID); ok {
			...
			return nil, fmt.Errorf("unexpected Video_play_range.ID type: %T", _spec.ID.Value)
		}
	}
...
func (vprc *VideoPlayRangeCreate) createSpec() (*Video_play_range, *sqlgraph.CreateSpec) {
	var (
		_node = &Video_play_range{config: vprc.config}
		_spec = sqlgraph.NewCreateSpec(video_play_range.Table, sqlgraph.NewFieldSpec(video_play_range.FieldID, field.TypeString))
	)
...
// VideoPlayRangeCreateBulk is the builder for creating many Video_play_range entities in bulk.
...
// Save creates the Video_play_range entities in the database.
func (vprcb *VideoPlayRangeCreateBulk) Save(ctx context.Context) ([]*Video_play_range, error) {
	if vprcb.err != nil {
		return nil, vprcb.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(vprcb.builders))
	nodes := make([]*Video_play_range, len(vprcb.builders))
...
func (vprcb *VideoPlayRangeCreateBulk) SaveX(ctx context.Context) []*Video_play_range {
