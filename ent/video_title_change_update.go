...
// VideoTitleChangeUpdate is the builder for updating Video_title_change entities.
...
func (vtcu *VideoTitleChangeUpdate) Where(ps ...predicate.Video_title_change) *VideoTitleChangeUpdate {
...
func (vtcu *VideoTitleChangeUpdate) SetVideoID(id pulid.ID) *VideoTitleChangeUpdate {
...
func (vtcu *VideoTitleChangeUpdate) check() error {
	if _, ok := vtcu.mutation.VideoID(); vtcu.mutation.VideoCleared() && !ok {
		return errors.New(`ent: clearing a required unique edge "Video_title_change.video"`)
	}
	return nil
}
...
func (vtcu *VideoTitleChangeUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(video_title_change.Table, video_title_change.Columns, sqlgraph.NewFieldSpec(video_title_change.FieldID, field.TypeString))
...
// VideoTitleChangeUpdateOne is the builder for updating a single Video_title_change entity.
...
// SetVideoID sets the "video" edge to the Video entity by ID.
func (vtcuo *VideoTitleChangeUpdateOne) SetVideoID(id pulid.ID) *VideoTitleChangeUpdateOne {
...
// Where appends a list predicates to the VideoTitleChangeUpdate builder.
func (vtcuo *VideoTitleChangeUpdateOne) Where(ps ...predicate.Video_title_change) *VideoTitleChangeUpdateOne {
...
// Save executes the query and returns the updated Video_title_change entity.
func (vtcuo *VideoTitleChangeUpdateOne) Save(ctx context.Context) (*Video_title_change, error) {
	return withHooks(ctx, vtcuo.sqlSave, vtcuo.mutation, vtcuo.hooks)
}

// SaveX is like Save, but panics if an error occurs.
func (vtcuo *VideoTitleChangeUpdateOne) SaveX(ctx context.Context) *Video_title_change {
...
func (vtcuo *VideoTitleChangeUpdateOne) sqlSave(ctx context.Context) (_node *Video_title_change, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "Video_title_change.id" for update`)}
	}
	...
	if fields := vtcuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			...
			if f != video_title_change.FieldID {
	...
	_node = &Video_title_change{config: vtcuo.config}
