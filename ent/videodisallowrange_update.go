...
// VideoDisallowRangeUpdate is the builder for updating VideoDisallowRange entities.
...
func (vdru *VideoDisallowRangeUpdate) Where(ps ...predicate.VideoDisallowRange) *VideoDisallowRangeUpdate {
...
func (vdru *VideoDisallowRangeUpdate) SetVideoID(id string) *VideoDisallowRangeUpdate {
...
func (vdru *VideoDisallowRangeUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(videodisallowrange.Table, videodisallowrange.Columns, sqlgraph.NewFieldSpec(videodisallowrange.FieldID, field.TypeString))
	...
	if value, ok := vdru.mutation.AddedStartSeconds(); ok {
		_spec.AddField(videodisallowrange.FieldStartSeconds, field.TypeInt, value)
	}
	...
	if value, ok := vdru.mutation.AddedEndSeconds(); ok {
		_spec.AddField(videodisallowrange.FieldEndSeconds, field.TypeInt, value)
	}
...
// VideoDisallowRangeUpdateOne is the builder for updating a single VideoDisallowRange entity.
...
// SetVideoID sets the "video" edge to the Video entity by ID.
func (vdruo *VideoDisallowRangeUpdateOne) SetVideoID(id string) *VideoDisallowRangeUpdateOne {
...
// Where appends a list predicates to the VideoDisallowRangeUpdate builder.
func (vdruo *VideoDisallowRangeUpdateOne) Where(ps ...predicate.VideoDisallowRange) *VideoDisallowRangeUpdateOne {
...
// Save executes the query and returns the updated VideoDisallowRange entity.
func (vdruo *VideoDisallowRangeUpdateOne) Save(ctx context.Context) (*VideoDisallowRange, error) {
	return withHooks(ctx, vdruo.sqlSave, vdruo.mutation, vdruo.hooks)
}

// SaveX is like Save, but panics if an error occurs.
func (vdruo *VideoDisallowRangeUpdateOne) SaveX(ctx context.Context) *VideoDisallowRange {
...
func (vdruo *VideoDisallowRangeUpdateOne) sqlSave(ctx context.Context) (_node *VideoDisallowRange, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "VideoDisallowRange.id" for update`)}
	}
	...
	if fields := vdruo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			...
			if f != videodisallowrange.FieldID {
	...
	_node = &VideoDisallowRange{config: vdruo.config}
